...
export class RateLimitMiddleware implements NestMiddleware {
    ...
    async use(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            res.setHeader('X-RateLimit-Remaining', Math.max(0, this.MAX_REQUESTS_PER_WINDOW - current));
            res.setHeader('X-RateLimit-Reset', Date.now() + this.WINDOW_SIZE_SECONDS * 1000);
