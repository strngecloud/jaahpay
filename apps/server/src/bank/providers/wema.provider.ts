...
export class WemaProvider implements IBankProvider {
  ...
  async validateAccount(
    ...
    try {
      ...
      return {
        ...
        bankCode,
        accountName:
          response.data.result?.accountName ||
          response.data.result?.account_name,
        valid: isValid,
      };
    } catch (error: any) {
  ...
  async transfer(request: BankTransferRequest): Promise<BankTransferResponse> {
    try {
      ...
      return {
        ...
        reference:
          response.data.result?.platformTransactionReference ||
          request.reference,
        status:
          response.data.result?.status || (isSuccessful ? 'pending' : 'failed'),
  ...
  async getTransferStatus(reference: string): Promise<BankTransferResponse> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get(
          `${this.apiUrl}/OpenApiTransfer/transfer-status/${reference}`,
          {
            headers: {
              'Ocp-Apim-Subscription-Key': this.apiKey,
            },
      ...
      throw new BankApiException('Wema', 'Transfer status check failed', error);
    }
  }
  ...
  async listBanks(): Promise<BankInfo[]> {
    try {
      ...
      const banks: BankInfo[] = (response.data.result || [])
        .filter(
          (b: { bankCode?: string; bankName?: string }) =>
