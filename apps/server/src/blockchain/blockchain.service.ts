...
export class BlockchainService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    const processorKey = this.configService.get<string>(
      'PROCESSOR_WALLET_PRIVATE_KEY',
    );
    ...
    if (
      ...
      this.processorAccount = privateKeyToAccount(
        processorKey as `0x${string}`,
      );
  ...
  private startEventListeners() {
    ...
    if (
      celoContractAddress &&
      celoContractAddress !== '0x0000000000000000000000000000000000000000'
    ) {
    ...
    if (
      baseContractAddress &&
      baseContractAddress !== '0x0000000000000000000000000000000000000000'
    ) {
  ...
  async handleTimedOutSpend(spendId: string, chain: Chain): Promise<string> {
    try {
      ...
      this.logger.error(
        `Failed to handle timeout for ${spendId}:`,
        error.message,
      );
      throw error;
    }
