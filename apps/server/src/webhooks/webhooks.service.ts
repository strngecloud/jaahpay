...
export class WebhooksService {
    ...
    async processWemaWebhook(payload: any): Promise<void> {
        ...
        if (alreadyProcessed) {
            this.logger.warn(
                `Webhook already processed: ${transactionReference}`,
            );
            return;
        }
        ...
        if (status === 'Successful' || status === 'Success') {
            ...
            await this.handleFailedTransfer(spend, payload.message || 'Transfer failed');
        }
    }
    ...
    async processPaystackWebhook(payload: any): Promise<void> {
        ...
        if (!spend) {
            this.logger.error(
                `Spend not found for webhook reference: ${reference}`,
            );
            return;
        }
    ...
    private async handleSuccessfulTransfer(
        ...
        try {
            ...
            this.logger.log(
                `Bank transfer successful for spend ${spend.spendId}`,
            );
