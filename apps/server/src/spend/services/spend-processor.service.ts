...
export class SpendProcessorService {
    ...
    private async executeBankTransfer(spend: SpendEntity): Promise<void> {
        ...
        const transferRequest: BankTransferRequest = {
            ...
            narration: spend.narration || `Jahpay spend: ${spend.spendId.slice(0, 10)}`,
    ...
    async getSpendHistory(
        ...
        page: number = 1,
        limit: number = 20,
    ): Promise<{ spends: SpendEntity[]; total: number; page: number; pages: number }> {
     ...
     */
    async cancelSpend(spendId: string, userAddress: string): Promise<SpendEntity> {
